...
const ShowInvoice: FC = () => {
  ...
  const findInvoice = useCallback(() => {
    const foundInvoice = invoices?.find(
      (invoiceInFind: Invoice) => invoiceInFind.id === Number(id)
    ) as Invoice;
  ...
  const { number, createdAt, paymentDate, dueDate, sum, paid } =
    invoice as Invoice;
  return (
    <>
      {loading ? (
        ...
        <section className="singleinv">
          ...
          <button
            className={`button__payonline ${
              paid ? "button__payonline--paid" : ""
            }`}
          >
            Pay online
          </button>
